...
{
	private static voce.SpeechSynthesizer mSynthesizer = null;
	private static voce.SpeechRecognizer mRecognizer = null;
	...
	{
		voce.Utils.setPrintDebug(false);
		voce.Utils.log("debug", "Beginning initialization");

		if (!initSynthesis && !initRecognition)
		{
			voce.Utils.log("warning", "Synthesizer and recognizer are both"
				+ "uninitialized.");
		}
		...
		if (initSynthesis)
		{
			...
			voce.Utils.log("", "Initializing synthesizer");
			mSynthesizer = new voce.SpeechSynthesizer("Kevin16");
		}
